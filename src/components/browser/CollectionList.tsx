...
export const CollectionList = ({ page, onPageChange }: CollectionListProps) => {
  ...
  if (error) {
    return (
      <Card>
        <CardContent className="py-8 text-center text-destructive">Error loading collections: {error.message}</CardContent>
      </Card>
    );
  }
  ...
  if (!data?.entities.length) {
    return (
      <Card>
        <CardContent className="py-8 text-center text-muted-foreground">No collections found.</CardContent>
      </Card>
    );
  }
